...
For each covariate $Z_i$, `hte_test_conditional(y, Tr, Z, S = i)` tests $H_0^c(\{i\})$: the treatment effect does not depend on $Z_i$, conditionally on the rest of the covariates. The test involves a random sample split, so we repeat it a number of times and combine the p-values by their harmonic mean, which controls the type-I error at (approximately) twice the nominal level (Wilson, 2019) while being robust to the choice of any single split.
