...
treatment effect
``` math
 \tau(Z) := \mathbb{E}[Y(t=1) - Y(t=0) \mid Z]
```
...
$`\alpha`$ level is small in the sense that
$`\text{Type-I error} / \alpha \to 1`$ holds as $`\alpha \to 0`$ under
the null, even when the $`p`$-values are computed on the same sample and
